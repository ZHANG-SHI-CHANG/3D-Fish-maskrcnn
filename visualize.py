...
def my_display_instances(image, boxes, masks, class_ids, class_names,
                      scores=None,colors=None, fish_num=None,title="",
                      figsize=(8, 8), ax=None,name=''):
    """
    ...
    for i,class_name in enumerate(class_names):
        class_count.append(np.sum((class_ids==i).astype(np.uint8)))
    caption = '{}:{} '.format('ALL',np.sum(class_count))
    for i,(_name,count) in enumerate(zip(class_names,class_count)):
    '''
    caption = '{}:{} '.format('ALL',fish_num[0])
    for i,(_name,count) in enumerate(zip(class_names,fish_num)):
        if i>=1 and i!=5:
    ...
    for i in range(N):
        ...
        caption = " {} {} ".format(label, '%s*%s' % (x2 - x1,y2 - y1))
        ax.text(x1, y1, caption,
                color='w', size=8, backgroundcolor="r",clip_on=True)
...
def my_display_instances1(image, boxes, class_ids, class_names,
                      scores=None,colors=None, fish_num=None,title="",
                      figsize=(8, 8), ax=None,name=''):
    """
    boxes: [num_instance, (y1, x1, y2, x2, class_id)] in image coordinates.
    ...
    for i in range(N):
        ...
        score = scores[i] if scores is not None else None
        label = class_names[class_id]
        caption = " {} {} ".format(label, '%s*%s' % ( int((x2 - x1)/20),int((y2 - y1)/20)))
        ax.text(x1, y1, caption,
                color='w', size=8, backgroundcolor=color,clip_on=True)
