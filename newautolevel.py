...
class AutoLevel():
    ...
    def autolevel(self,img,lowcut = 0.01,highcut = 0.001):
...
def main_Frame():
    test = AutoLevel()
    test.StartFrame(test.ReadImage('0000075.jpg'))
...
if __name__ == '__main__':
    ...
    data_paths = os.path.join(root,'data')
    result_paths = os.path.join(root,'result')
    is not os.path.exists(result_paths):
        os.makedirs(result_paths)
    
    for i,data_path in enumerate(glob.glob(os.path.join( data_paths,'*' ))):
        result_path_name = data_path.split('\\')[-1]
        result_path = os.path.join(result_paths,result_path_name)
        if not os.path.exists(result_path):
            os.makedirs(result_path)
        for j,image_path in enumerate(os.path.join( data_path,'*.jpg' )):
            image_name = image_path.split('\\')[-1]
            image = cv2.imread(image_path)
            result = Dehaze(image)
            cv2.imwrite(os.path.join(result_path,image_name),result)
